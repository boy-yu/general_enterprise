...
class _RefreState extends State<Refre> with TickerProviderStateMixin {
  ...
  AnimationController _keepAnimationController;
  Animation<double> _keepSizeFactor;
    ...
    if (scrollnotification.metrics.axis == Axis.vertical) {
      ...
      if (start &&
        ...
        if (scrollnotification is OverscrollNotification) {
          ...
          _scrollValue = scrollnotification.metrics.pixels;
        }
        if (!refresh) {
  ...
  void initState() {
    ...
    _keepAnimationController =
        AnimationController(vsync: this, duration: Duration(milliseconds: 300));
    _keepSizeFactor =
        Tween(begin: 1.0, end: 0.0).animate(_keepAnimationController);
    if (widget.initRefresh) {
      _show();
    }
    if (widget.fisetRequest) {
      widget.onRefresh();
    }
  }
      ...
      widget.onRefresh().then((value) {
        if (mounted) {
          _keepAnimationController?.forward()?.then((value) {
            _scrollValue = 0;
            refresh = false;
            _keepAnimationController = AnimationController(
                        ...
                        children: [
                          Text('正在努力加载',
                              style: TextStyle(
                                  ...
                                  fontSize: _scrollValue.abs() > 50
                                      ? 14
                                      : .3 * _scrollValue.abs())),
                          SizedBox(width: size.width * 20),
                          Image.asset('assets/images/home_toggle.gif',
                              height: _scrollValue.abs() > 50
                                  ? 50
                                  : _scrollValue.abs())
                        ]))
                : SizeTransition(
