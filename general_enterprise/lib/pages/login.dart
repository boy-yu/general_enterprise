    ...
    loginDio.request(type: 'post', url: Interface.loginUrl, data: {
      ...
      if (value is Map) {
        ...
        await myprefs.setString(
            'username', value['systemUser']['username'] ?? '');
        // 头像
        await myprefs.setString('avatar', value['systemUser']['avatar'] ?? '');
        // 描述
        await myprefs.setString(
            'description', value['systemUser']['description'] ?? '');
        // 电子邮箱
...
class _InputKeyState extends State<InputKey> {
  ...
  Widget build(BuildContext context) {
    return TextField(
      keyboardType: widget.obscureText != null
      ...
      decoration: InputDecoration(
        enabledBorder: UnderlineInputBorder(
          // 不是焦点的时候颜色
          borderSide:
              BorderSide(color: Color(0xffECECEC), width: size.width * 2),
        ),
