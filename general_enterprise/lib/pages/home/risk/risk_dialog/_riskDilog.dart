...
class RiskDilog extends StatefulWidget {
  final Function callback;
  final int threeId;
  final String riskItemtitle;
  final bool operable;
  RiskDilog({this.threeId, this.callback, this.riskItemtitle, this.operable});
  @override
  _RiskDilogState createState() => _RiskDilogState();
}

class _RiskDilogState extends State<RiskDilog> {
  Map threeDetailsMap = {
    "riskName": "",
    "riskLevel": 0,
        ...
        if (value != null) {
          ...
          if (threeDetailsMap['initialRiskLevel'] != null) {
            switch (threeDetailsMap['initialRiskLevel']) {
              case 0:
                initialRiskLevel = '暂无数据';
                break;
              case 1:
                initialRiskLevel = '重大风险';
                break;
              case 2:
                initialRiskLevel = '较大风险';
                break;
              case 3:
                initialRiskLevel = '一般风险';
                break;
              case 4:
                initialRiskLevel = '低风险';
                break;
              default:
            }
          }
      ...
      child: Stack(
        children: [
          ...
          BuildDragWidget(
              ...
              threeId: widget.threeId,
              riskItemtitle: widget.riskItemtitle,
...
class RiskDropDown extends StatefulWidget {
  RiskDropDown(
      {this.callback, this.threeId, this.riskItemtitle, this.operable});
  final Function callback;
  final int threeId;
  final String riskItemtitle;
  final bool operable;
  @override
  _RiskDropDownState createState() => _RiskDropDownState();
}

class _RiskDropDownState extends State<RiskDropDown> {
  // riskunameController
  TextEditingController riskunameController = TextEditingController();
  ...
  saveData() async {
    if (_counter.submitDates["executionUrls"] != null &&
      ...
      riskControlVo["executionUrl"] =
          _counter.submitDates["executionUrls"][0]["value"].join("|");
    }
    riskControlVo["opinion"] = riskunameController.text;
    riskControlVo["executionStatus"] = isFull ? 1 : 2;
    List forids = [];
    if (!isFull) {
      ...
      riskControlVo["fourIds"] = forids;
      setState(() {});
    }
    riskControlVo['threeId'] = widget.threeId;
    if (riskControlVo['executionUrl'] == '') {
      Fluttertoast.showToast(msg: "请拍照后再提交");
    } else if (riskControlVo['opinion'] == '') {
      Fluttertoast.showToast(msg: "请先填写描述");
    } else {
      await myDio.request(
          type: 'post', url: Interface.setriskdata, data: riskControlVo);
        ...
        child: Column(
          children: <Widget>[
            ...
            Container(
              ...
              child: TextField(
                autofocus: false,
                controller: riskunameController,
