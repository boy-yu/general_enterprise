...
class ExamSpotPic extends StatefulWidget {
  final List<ExamCacheType> data;
  final bool formalExam;
  final int id, stage, type;
  final Map submitStudyRecords;
  final List picQuestionData;
  const ExamSpotPic(this.data,
      {Key key,
      this.formalExam = false,
      @required this.id,
      this.submitStudyRecords, this.stage, this.type, this.picQuestionData})
      : super(key: key);
  @override
  _ExamSpotPicState createState() => _ExamSpotPicState();
}
...
class _ExamSpotPicState extends State<ExamSpotPic> {
  List<Offset> _points = <Offset>[];
  double width=0;
  double height =0;
  double cWidth =size.width * 200;
  double cHeight =size.width * 200;
  double dHeight;
  double dWidth;
  bool  isDrag = true;
  bool  first = true;
  ...
  void initState() {
    ...
    _points.add(Offset(0, 0));
    _getPicQuestionList();
    // ImageLoader.load("assets/images/emergencyRescue.png").then((image2) {
    //   setState(() {
    //     this.image = image2;
    //   });
    // });
  ...
  _getPicQuestionList(){
    ...
    //   if(value is List){
    //     data = value;
    //     ImageLoader.load(data[imageIndex]['picUrl'].toString().split("|")[0]).then((image2) {
    //       setState(() {
    //         this.image = image2;
    //       });
    //     });
    //   }
    // });
    ImageLoader.load(widget.picQuestionData[imageIndex]['picUrl'].toString().split("|")[0]).then((image2) {
                                  setState(() {
                                    this.image = image2;
                                  });
                                });
  ...
  bool hitPoint(Offset offset,double radius,Offset down){
    print('down.dx>'+ (down.dx>offset.dx-radius).toString());
    print('down.dx<'+ (down.dx<offset.dx+radius).toString());
    print('down.dy>'+ (down.dy>offset.dy-radius).toString());
    print('down.dy<'+ (down.dy<offset.dy+radius).toString());
  ...
  Widget build(BuildContext context) {
    ...
    return MyAppbar(
      ...
      child: Container(
        color: Colors.white,
        child: widget.picQuestionData.isNotEmpty ? Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
              widget.picQuestionData[imageIndex]['problemDescription'].toString()
            ),
            Expanded(
              child: Stack(
                children: [
                    ...
                    child:Row(children: <Widget>[
                      GestureDetector(
                          child: Container(
                            ...
                            child: Text(
                              imageIndex < widget.picQuestionData.length-1 ? '下一题' : '完成考试',
                              style: TextStyle(
                                color: Colors.white,
          ...
          //                   image2.then((value) =>print('value' + value.toString())
                              
          //                   // (Navigator.push(context, new MaterialPageRoute(builder: (context) => ImageDetailPage(title: "image", image: value))))
          //                   );
          //                   print('dest------------------------' + dest.width .toString());
          //                   print('dest------------------------' + dest.height .toString());
          // //                   print('_points------------------------' + _points.toString());
          // //    print('_points------------------------' + _points[2].dx.toString());
          // //    print('_points------------------------' + _points[2].dy.toString());
                            submitData.add(
                              {
                                "companyId": widget.picQuestionData[imageIndex]['companyId'],
                                "doubleOneX": _points[2].dx / width,
                                "doubleOneY": _points[2].dy / height,
                                "doubleTwoX": (_points[2].dx + (dest.width / 4)) / width,
                                "doubleTwoY": (_points[2].dy + (dest.height / 4)) / height,
                                "picUrl": widget.picQuestionData[imageIndex]['picUrl'],
                                "pictureIdentification": widget.picQuestionData[imageIndex]['pictureIdentification'],
                                "problemDescription": widget.picQuestionData[imageIndex]['problemDescription'],
                              }
                            );
                            // print(submitData);
                            if(imageIndex < widget.picQuestionData.length-1){
...
class ImageLoader {
  ...
  static Future<flutterui.Image> load(String url) async {
    ui.ImageStream stream = new ui.NetworkImage(url)
...
class SignaturePainter extends CustomPainter {
  ...
  double cWidth;  //clip square width
  double cHeight;  //clip square height
